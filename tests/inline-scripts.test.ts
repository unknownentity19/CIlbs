...
import { GFX_LITE_SCRIPT, INLINE_SCRIPT_HASHES } from "@/lib/inline-scripts";
 ...
 * snippet in production — which is how the touch-hardware guard silently
 * stopped working on /dashboard and /studio. This pins it.
 */
...
describe("inline script hashes", () => {
  it("matches the CSP source for each snippet", () => {
    expect(INLINE_SCRIPT_HASHES).toEqual([csp(GFX_LITE_SCRIPT)]);
  });

  it("keeps the snippets free of characters that break an inline script", () => {
    for (const source of [GFX_LITE_SCRIPT]) {
